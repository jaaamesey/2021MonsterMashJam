...
func handle_command(command : String):
	...
	match command:
		...
		"set_checkpoint":
			if len(parameters) < 1:
				DialogueManager.variable_dict["checkpoint"] = DialogueManager.get_current_id()
			else:
				DialogueManager.variable_dict["checkpoint"] = parameters[0]

		"game_over":
			if !DialogueManager.variable_dict.has("checkpoint"):
				DialogueManager.variable_dict["checkpoint"] = "START"
			visible = false
			DialogueManager.go_to_dialogue(DialogueManager.variable_dict["checkpoint"])
			Engine.time_scale = 0
			pause_timer += 2
